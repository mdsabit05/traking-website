  ...
  tr.innerHTML = `
    ...
    <td class="controls">
      <div style="display:flex;gap:6px;justify-content:center">
        <button class="add-sub small" onclick="addSubtopic('${mainId}', this)">➕ Sub</button>
        <button class="collapse-btn small" onclick="toggleSub('${mainId}', this)">▾</button>
...
function createSubRow(mainId, subId, title='New Subtopic', start=isoToday(), due=isoToday(), target=0, completed=0){
  ...
  tr.innerHTML = `
    ...
    <td class="progressCell">0%</td>
    <td class="statusCell"><span class="status not-started">Not Started</span></td>
    <td class="controls"><button class="delete-btn" onclick="deleteRow(this)">❌</button></td>
  `;
  return tr;
}
...
function saveData(){
  ...
  const data = rows.map(r => {
    const obj = { type: r.dataset.type, id: r.dataset.id };
    if (r.dataset.type === 'main'){
      obj.title = r.querySelector('.main-title')?.innerText || '';
      obj.start = r.querySelectorAll('.date-input')[0]?.value || '';
      obj.due = r.querySelectorAll('.date-input')[1]?.value || '';
    } else {
      obj.parent = r.dataset.parent;
      obj.title = r.querySelector('.sub-title')?.innerText || '';
      obj.start = r.querySelectorAll('.date-input')[0]?.value || '';
      obj.due = r.querySelectorAll('.date-input')[1]?.value || '';
      obj.target = r.querySelector('.target-input')?.value || 0;
      obj.completed = r.querySelector('.completed-input')?.value || 0;
    }
    return obj;
  });
...
function loadData(){
  const saved = localStorage.getItem('progressTrackerData');
  if (!saved) return false;
  const data = JSON.parse(saved);
  tbody.innerHTML = ''; // clear table
  data.forEach(item => {
    if (item.type === 'main'){
      tbody.appendChild(createMainRow(item.id, item.title, item.start, item.due));
    } else {
      tbody.appendChild(createSubRow(item.parent, item.id, item.title, item.start, item.due, item.target, item.completed));
    }
  });
  return true;
}
...
function expandAll(){
  tbody.querySelectorAll('tr[data-type="main"]').forEach(m => {
    ...
    const id=m.dataset.id; const btn = m.querySelector('.collapse-btn'); if(btn) btn.textContent='▾';
...
function updateAll(){
  const rows = Array.from(tbody.querySelectorAll('tr'));
  rows.forEach(r=>{ if (r.dataset.type === 'sub') computeProgressForSub(r); });
  rows.forEach(r=>{ if (r.dataset.type === 'main') computeProgressForMain(r); });
  updateCharts();
  saveData();  // 🔥 Auto-save
}
...
function updateCharts(){
  ...
  const overallLabels = [], overallData = [];
  mainRows.forEach(m => {
    const stats = computeProgressForMain(m);
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if (!loadData()){
    addMainGoal();
    const firstMain = tbody.querySelector('tr[data-type="main"]');
    if (firstMain) addSubtopic(firstMain.dataset.id);
  }
  updateAll();

  tbody.addEventListener('input', e => { if (e.target.closest('tr')) updateAll(); });
  tbody.addEventListener('blur', e => { if (e.target.closest('tr')) updateAll(); }, true);
